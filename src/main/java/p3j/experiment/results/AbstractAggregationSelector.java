 ...
 * a {@link BasicResults} object from the lists in {@link ResultsOfTrial}.
 * 
...
public abstract class AbstractAggregationSelector implements
	...
	private final IOutputVariableSelector basicResultsSelector;

	/** The trial results selector. */
	private final ISubPopulationSelector trialResultsSelector;
	 ...
	 */
	public AbstractAggregationSelector(IOutputVariableSelector resultsSelector,
	    ISubPopulationSelector trialSelector, int generationForSelection) {
		basicResultsSelector = resultsSelector;
		trialResultsSelector = trialSelector;
	...
	protected Matrix2D select(ResultsOfTrial resultsOfTrial) {
		return basicResultsSelector.select(trialResultsSelector.select(
	...
	protected String getFileName() {
		return trialResultsSelector.getPrefix()
		    + (generation == -1 ? "" : "_gen_" + generation) + "_"
		    + basicResultsSelector.getSuffix();
	}
	...
	protected double[] sumPerElement(List<double[]> additionArrays,
		...
		// (additionArrays.size() > 0 ? additionArrays.get(0).length :
		// subtractionArrays.get(0).length)
...
/**
 * Selector for a certain element of the basic results.
 * 
 * @see BasicResults
 * 
 * @author Christina Bohk
...
/**
 * Selector for a basic result set from the overall results of a trial.
 * 
 * @see ResultsOfTrial
 * @see BasicResults
 * 
