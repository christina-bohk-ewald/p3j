...
public class ResultExport {
	...
	/**
	 * The name of the directory in which all files required for report generation
	 * are situated after deployment.
	 */
	private static final String DEPLOYMENT_DIR_NAME = "report_template";
	...
	public ResultExport(ProjectionModel projectionModel, File targetDirectory,
		...
		// Check whether the directory structure for development exists, otherwise
		// choose deployment directory
		File srcDir = new File("./"
		    + this.getClass().getPackage().getName().replace('.', '/'));
		templateDirectory = srcDir.exists() ? srcDir : new File("./"
		    + DEPLOYMENT_DIR_NAME);
	...
	private void copyPlottingLib() throws IOException {
		...
		FileChannel source = new FileInputStream(
		    templateDirectory.getAbsolutePath() + File.separator + R_LIBRARY)
		    .getChannel();
		FileChannel destination = new FileOutputStream(targetDir.getAbsolutePath()
		    + File.separator + R_LIBRARY).getChannel();
		...
		try {
			...
			attemptClosing(source, destination);
		}
	...
	/**
	 * Attempts to close all given channels.
	 * 
	 * @param channels
	 *          the channels
	 * 
	 ...
	 *           Signals that an I/O exception has occurred.
	 */
	private void attemptClosing(Channel... channels) throws IOException {
