...
public class ProjectionTreePanel extends AbstractNavigationPanel implements
  ...
  protected SetNode addSetSubTree(SetTypeNode root, Set set, SetType setType) {
    ...
    Map<Integer, List<ParameterInstance>> generationMap = new HashMap<>();
    ...
    List<Integer> genIndices = new ArrayList<>(generationMap.keySet());
  ...
  private int retrieveGeneration(ParameterInstance instance) {
    ...
    if (instance.getParameter().isGenerationDependent()
  ...
  protected void addPopulationsSubTree(ProjectionTreeNode<?> root, Set set,
      List<ParameterInstance> instances) {
    for (SubPopulation subPopulation : pTreeRoot.getEntity()
        .getSubPopulationModel().getSubPopulations()) {
      List<ParameterInstance> popInst = ProjectionModel
          .filterParamInstancesBySubPopulation(instances, subPopulation);
      if (popInst.isEmpty())
        continue;
      SubPopulationNode popNode = new SubPopulationNode(subPopulation);
      root.add(popNode);
   ...
   */
  protected void addInstancesSubTree(SubPopulationNode root, Set set,
      List<ParameterInstance> instances) {
  ...
  protected void cleanTree(ProjectionTreeNode<?> node) {
    ...
    if (node.getChildCount() == 0
        && (node instanceof SubPopulationNode || node instanceof GenerationNode)) {
      getTreeModel().removeNodeFromParent(node);
    }
  }
